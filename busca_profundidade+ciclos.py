...
def grau_vertices(vertices, arestas, nao_direcionado=False):
    graus = {}
    for v in vertices:
        saida = len([1 for (orig, _) in arestas if orig == v])
        entrada = len([1 for (_, dest) in arestas if dest == v])
...
# -----------------------------------------
#  NOVAS FUNÇÕES: DFS e DETECÇÃO DE CICLOS
# -----------------------------------------

def dfs(vertices, arestas, inicio, visitados=None):
    ...
    for viz in vizinhos(vertices, arestas, inicio):
        if viz not in visitados:
            dfs(vertices, arestas, viz, visitados)


def detectar_ciclo(vertices, arestas):
    visitado = set()
    pilha_rec = set()  # conjunto que detecta ciclos

    def dfs_ciclo(v):
        visitado.add(v)
        pilha_rec.add(v)

        for viz in vizinhos(vertices, arestas, v):
...
def main():
    ...
    while True:
        ...
        if opc == "1":
            ...
            print("DFS:", end=" ")
            dfs(vertices, arestas, inicio)
            print()

        elif opc == "11":
            if detectar_ciclo(vertices, arestas):
